...
export function ConfirmPaymentModal({ open, amount, merchant, onConfirm, onCancel }: ConfirmPaymentModalProps) {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
      <div className="w-full max-w-sm rounded-2xl bg-secondary border border-white/15 shadow-xl px-6 py-5 text-left space-y-3">
        ...
        <p className="text-sm text-secondary">
          ...
          <span className="font-semibold text-primary">{amount || '—'} USDC</span> from your connected wallet. The
          amount will be credited to the merchant via the shared pool (Base Sepolia).
        </p>
